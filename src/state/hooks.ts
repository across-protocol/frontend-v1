...
export function useSend() {
  ...
  const { fromChain, toChain, toAddress, amount, token, error, currentlySelectedFromChain, currentlySelectedToChain  } =
  ...
  const send = useCallback(async () => {
    ...
    try {
      const depositBox = getDepositBox(currentlySelectedFromChain.chainId, signer);
      const isETH = token === ethers.constants.AddressZero;
      const value = isETH ? amount : ethers.constants.Zero;
      const l2Token = isETH ? TOKENS_LIST[currentlySelectedFromChain.chainId][0].address : token;
      const { instantRelayFee, slowRelayFee } = fees;
      const timestamp = block.timestamp - (1000 * 60 * 10);
